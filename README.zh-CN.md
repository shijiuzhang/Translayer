...
- 文本翻译阶段显示当前页、已完成页数和文本块数量。
- 执行已批准的图片计划时显示已完成图片数，以及 OCR、翻译、擦除、
  重绘、生成、复用和质量检查等当前子阶段。
- OpenAI 兼容引擎支持 Moonshot Kimi K2.5/K2.6，会自动关闭 thinking，
  并避免发送模型不支持的 `temperature: 0`。
- 上游 API 报错会包含响应正文，便于排查鉴权、模型名和参数问题。
